...
helm install \
  ...
  --set Worker.Environment[1].name="SPARK_WORKER_OPTS",Worker.Environment[1].value="-Dspark.worker.cleanup.enabled=true -Dspark.worker.cleanup.interval=10  -Dspark.worker.cleanup.appDataTtl=10" \
  --namespace spark \
  --name spark-cluster \
  ./spark
